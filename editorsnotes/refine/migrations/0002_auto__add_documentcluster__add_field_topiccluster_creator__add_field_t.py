...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.create_table('refine_documentcluster', (
        ...
        db.send_create_signal('refine', ['DocumentCluster'])

        # Adding M2M table for field documents on 'DocumentCluster'
        db.create_table('refine_documentcluster_documents', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('documentcluster', models.ForeignKey(orm['refine.documentcluster'], null=False)),
            ('document', models.ForeignKey(orm['main.document'], null=False))
        ))
        db.create_unique('refine_documentcluster_documents', ['documentcluster_id', 'document_id'])

        # Adding field 'TopicCluster.creator'
        db.add_column('refine_topiccluster', 'creator', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'], null=True, blank=True), keep_default=False)

        # Adding field 'TopicCluster.created'
        db.add_column('refine_topiccluster', 'created', self.gf('django.db.models.fields.DateTimeField')(auto_now_add=True, default=datetime.datetime(2012, 7, 18, 16, 42, 40, 798266), blank=True), keep_default=False)

        # Adding field 'TopicCluster.marked_for_merge'
        db.add_column('refine_topiccluster', 'marked_for_merge', self.gf('django.db.models.fields.BooleanField')(default=False), keep_default=False)

        # Adding field 'TopicCluster.message'
        db.add_column('refine_topiccluster', 'message', self.gf('django.db.models.fields.TextField')(null=True, blank=True), keep_default=False)

        # Removing M2M table for field clustered_topics on 'TopicCluster'
        db.delete_table('refine_topiccluster_clustered_topics')

        # Adding M2M table for field topics on 'TopicCluster'
        db.create_table('refine_topiccluster_topics', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('topiccluster', models.ForeignKey(orm['refine.topiccluster'], null=False)),
            ('topic', models.ForeignKey(orm['main.topic'], null=False))
        ))
        db.create_unique('refine_topiccluster_topics', ['topiccluster_id', 'topic_id'])


    def backwards(self, orm):
        
        # Deleting model 'DocumentCluster'
        db.delete_table('refine_documentcluster')

        # Removing M2M table for field documents on 'DocumentCluster'
        db.delete_table('refine_documentcluster_documents')

        # Deleting field 'TopicCluster.creator'
        db.delete_column('refine_topiccluster', 'creator_id')

        # Deleting field 'TopicCluster.created'
        db.delete_column('refine_topiccluster', 'created')

        # Deleting field 'TopicCluster.marked_for_merge'
        db.delete_column('refine_topiccluster', 'marked_for_merge')

        # Deleting field 'TopicCluster.message'
        db.delete_column('refine_topiccluster', 'message')

        # Adding M2M table for field clustered_topics on 'TopicCluster'
        db.create_table('refine_topiccluster_clustered_topics', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('topiccluster', models.ForeignKey(orm['refine.topiccluster'], null=False)),
            ('topic', models.ForeignKey(orm['main.topic'], null=False))
        ))
        db.create_unique('refine_topiccluster_clustered_topics', ['topiccluster_id', 'topic_id'])
