...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for source in orm.Source.objects.all():
            source.last_updater = source.creator
            source.last_updated = source.created
            source.save()
        for transcript in orm.Transcript.objects.all():
            transcript.last_updater = transcript.creator
            transcript.last_updated = transcript.created
            transcript.save()
        for footnote in orm.Footnote.objects.all():
            footnote.last_updater = footnote.creator
            footnote.last_updated = footnote.created
            footnote.save()
