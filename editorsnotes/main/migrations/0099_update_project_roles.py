...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for project in orm['main.project'].objects.all():
            old_groups = orm['auth.group'].objects.filter(
                name__startswith='{}_'.format(project.slug))
            for group in old_groups:
                group.delete()
            group = orm['auth.group'].objects.create(
                name=u'{}-{}'.format(project.slug, 'Editor'))
            editor_groups[project] = orm['main.projectrole'].objects.create(
                project=project, role='Editor', group=group, is_super_role=True)

        for profile in orm['main.userprofile'].objects.all():
            if profile.affiliation is None:
                continue
            editor_groups[profile.affiliation].group.user_set.add(profile.user)
