    ...
    def forwards(self, orm):
        for source in orm.Source.objects.all():
            document = orm.Document(
                ...
                ordering=source.ordering, url=source.url,
                creator=source.creator, created=source.created,
                last_updater=source.last_updater, last_updated=source.last_updated)
            document.save()
            try:
                source._transcript.document = document
                source._transcript.save()
            except orm.Transcript.DoesNotExist:
                pass
            for scan in source.scans.all():
