...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        try:
            topic_type = orm['contenttypes.ContentType'].objects.get(app_label='main', model='topic')
            note_type = orm['contenttypes.ContentType'].objects.get(app_label='main', model='note')
            for topic in orm.Topic.objects.filter(article__isnull=False):
                topic.summary = topic.article.content
                for citation in orm.Citation.objects.filter(content_type=note_type, object_id=topic.article.id):
                    citation.content_type = topic_type
                    citation.object_id = topic.id
                    citation.save()
                for topic_assignment in orm.TopicAssignment.objects.filter(content_type=note_type, object_id=topic.article.id):
                    topic.related_topics.add(topic_assignment.topic)
                topic.save()
