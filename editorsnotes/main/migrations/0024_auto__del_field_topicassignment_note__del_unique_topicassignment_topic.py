...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Removing unique constraint on 'TopicAssignment', fields ['topic', 'note']
        db.delete_unique('main_topicassignment', ['topic_id', 'note_id'])

        # Deleting field 'TopicAssignment.note'
        db.delete_column('main_topicassignment', 'note_id')

        # Adding unique constraint on 'TopicAssignment', fields ['topic', 'object_id']
        db.create_unique('main_topicassignment', ['topic_id', 'object_id'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'TopicAssignment', fields ['topic', 'object_id']
        db.delete_unique('main_topicassignment', ['topic_id', 'object_id'])

        # We cannot add back in field 'TopicAssignment.note'
        raise RuntimeError(
            "Cannot reverse this migration. 'TopicAssignment.note' and its values cannot be restored.")

        # Adding unique constraint on 'TopicAssignment', fields ['topic', 'note']
        db.create_unique('main_topicassignment', ['topic_id', 'note_id'])
