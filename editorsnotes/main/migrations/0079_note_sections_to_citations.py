...
class Migration(DataMigration):
	...
	try:
        ...
        created = [f for f in orm['main.citation']._meta.local_fields \
                   if f.name == 'created'][0]
        updated = [f for f in orm['main.citation']._meta.local_fields \
        ...
        for ns in orm['main.notesection'].objects.select_related('note'):
            ...
            # Create a new citation identical to the note section
            citation = orm['main.citation'](
                document=ns.document,
                notes=ns.content,
                creator=ns.creator,
                created=ns.created,
                last_updater=ns.last_updater,
                last_updated=ns.last_updated,
                content_type=note_ct,
                object_id=ns.note.id
            )

            # When I initially made note sections distinct from citations, I
            # didn't change the reversion versions to reflect those changes.
            # Here, I'm finding the old versions of these note sections (when
            # they were citations), and updating them to point to the new
            # citation. Hurrah. Remind me not to break things in the data model
            # again.
            old_versions = orm['reversion.version'].objects\
                .select_related('revision')\
                .filter(
                    Q(content_type=citation_ct),
                    Q(serialized_data__contains='"document": %s' % ns.document_id) & \
                    Q(serialized_data__contains='"content_type": %s' % note_ct.id) & \
                    Q(serialized_data__contains='"object_id": %s' % ns.note_id)
                ).order_by('revision__date_created')

            # If there was an old version of this citation, change its creator &
            # creation timestamp accordingly.
            if len(old_versions):
                citation.creator = old_versions[0].revision.user or citation.creator
                citation.created = old_versions[0].revision.date_created

            # Update the database
            citation.save()
            old_versions.update(object_id=citation.id)
            ns.delete()
