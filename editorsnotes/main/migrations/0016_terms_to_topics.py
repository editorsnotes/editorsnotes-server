...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for term in orm.Term.objects.all():
            topic = orm.Topic(preferred_name=term.preferred_name, 
                              slug=term.slug, article=term.article,
                              creator=term.creator, created=term.created)
            topic.save()
            for alias in term.aliases.all():
                alias.topic = topic
                alias.save()
            for term_assignment in orm.TermAssignment.objects.filter(term__id=term.id):
                topic_assignment = orm.TopicAssignment(
                    topic=topic, note=term_assignment.note,
                    creator=term_assignment.creator, 
                    created=term_assignment.created)
                topic_assignment.save()
