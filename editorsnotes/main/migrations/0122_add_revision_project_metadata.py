...
def project_from_id(pid, orm):
    return orm['main.Project'].objects.get(id=pid)
project_from_id = memoize(project_from_id, {}, 1)

def affiliation_for_user(user, orm):
    user_groups = user.groups.values_list('id', flat=True)

    project = orm['main.Project'].objects\
            .select_related('projectrole', 'roles')\
            .filter(roles__group_id__in=user_groups)

    return project.get()
affiliation_for_user = memoize(affiliation_for_user, {}, 1)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        affiliated_models = [ct.id for ct in ContentType.objects.all()
                             if ct.model_class() is not None
                             and hasattr(ct.model_class(), 'get_affiliation')]

        for revision in orm['reversion.Revision'].objects.select_related('version_set').all():

            version_projects = revision.version_set\
                    .filter(serialized_data__contains='"project"')
            affiliated_versions = revision.version_set\
                    .filter(content_type_id__in=affiliated_models)
            if version_projects.count():
                data = json.loads(version_projects[0].serialized_data)[0]
                pid = data['fields']['project']
                project = project_from_id(pid, orm)

            elif affiliated_versions.count():
                project = None
                for version in affiliated_versions:
                    model = ContentType.objects.get(id=version.content_type_id).model_class()
                    try:
                        obj = model.objects.get(id=version.object_id)
            ...
            if project is None:
                project = affiliation_for_user(revision.user, orm)

            orm['main.RevisionProject'].objects.create(
                revision_id=revision.id, project_id=project.id)
    ...
    models = {
        ...
        'main.revisionproject': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'project': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['main.Project']"}),
            'revision': ('django.db.models.fields.related.OneToOneField', [], {'related_name': "'project_metadata'", 'unique': 'True', 'to': u"orm['reversion.Revision']"})
        },
        'main.scan': {
