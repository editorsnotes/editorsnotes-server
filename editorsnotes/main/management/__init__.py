...
def _get_project_permission_group():
    group, created = Group.objects\
        .get_or_create(name=PROJECT_PERMISSIONS_GROUP)
    if created:
...
def get_all_builtin_project_permissions(models=None):
    perms = set()
    models_to_search = apps.get_models() if models is None else models
    for model in models_to_search:
        ...
        perms.update({
            p for p in
            _get_project_specific_permissions(model, is_concretely_inherited)
        })
    return perms
